...
func TestCabinetEmailRegisterLogin(t *testing.T) {
	...
	id, err := a.CabinetEmailRegister(ctx, "User@Example.com", "secret1")
	if err != nil || id >= 0 {
	...
	if _, err := a.CabinetEmailRegister(ctx, "user@example.com", "secret1"); err == nil {
		t.Fatal("duplicate email must be rejected (case-insensitive)")
	}
	if lid, err := a.CabinetEmailLogin(ctx, "user@example.com", "secret1"); err != nil || lid != id {
